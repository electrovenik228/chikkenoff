...
API требует авторизацию. Для каждого пользователя нужно создать `EmployeeProfile` и указать:

- `role` - роль сотрудника.
- `primary_branch` - филиал сотрудника.

Роли `owner` и `director`, а также `superuser`, видят все филиалы. Остальные роли видят только данные своего `primary_branch`: заказы, KDS, смены, оплаты, сотрудников, клиентов, цены, склад и dashboard. Если у пользователя нет активного профиля или филиала, API вернет пустые списки.

Рабочие экраны POS/KDS/аналитики/склада являются филиальными. Для владельца или директора нужно передавать `?branch=<id>`, чтобы открыть конкретный филиал. Общая сетьвая аналитика будет вынесена в отдельную страницу позже.
